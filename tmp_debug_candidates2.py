...
for asin in ['B0FJT8JWRP', 'B0FRGJWHJ7', 'B0DRV5L8GC']:
    ...
    print('availability ids', [el.get('id') for el in soup.select('#availability, #outOfStock')])
    print('availability text', [d.get_text(' ', strip=True) for d in soup.select('#availability, #outOfStock')])
    ...
    def score(node):
        ...
        if re.search(r'\d', txt):
            score += 5
        return score
    scored = [(score(n), n.get_text(strip=True), n.name, [p.get('id') for p in n.parents if p.get('id')][:5], bool(n.find_parent('a')), [p.get('class') for p in n.parents if p.get('class')][:5]) for n in candidates]
    scored.sort(reverse=True, key=lambda x: x[0])
    for s, t, name, parent_ids, in_a, classes in scored[:20]:
        print('score', s, 'text', repr(t), 'name', name, 'parent_ids', parent_ids, 'in_a', in_a, 'classes', classes)
    print('---')
